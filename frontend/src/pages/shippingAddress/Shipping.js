...
function Shipping(props) {
  const { setOpen, shipOpen, shipSetOpen } = props;
  ...
  const [phoneNumber, setNumber] = useState(shipping?.phoneNumber || '');
  ...
  const [country, setCountry] = useState(shipping?.country || "Nigeria");
  ...
  const handleShipping = async (e) => {
    ...
    setLoading(true);
    if (shipOpen || shippingAddress === true) {
      const shippingData = {
        name: name,
  ...
  return (
    <div className="cartOpacity2" style={{ borderRadius: '20px' }}>
      <div
